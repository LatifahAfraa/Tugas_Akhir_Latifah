...
if (isset($_GET['rfid'])) { //jika parameter tersedia
    $response['waktu'] = 0;

    $rfid = $_GET['rfid']; //ambil nilai parameter dari alat
    $scan = (int) $_GET['scan'];
    ...
    if ($scan and !empty($rfid)) {
        ...
        $query_user = mysqli_query($konek, "SELECT * FROM user WHERE id_rfid='$rfid' AND verifikasi='1' AND status='1'");
